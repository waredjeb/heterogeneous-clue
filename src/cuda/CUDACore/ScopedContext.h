...
namespace impl {
// This class is intended to be derived by other ScopedContext*, not for general
// use
...
public:
  ...
  const T& get(const Product<T>& data) {
    synchronizeStreams(data.device(), data.stream(), data.isAvailable(),
                       data.event());
    return data.data_;
  }
...
protected:
  ...
  ScopedContextGetterBase(Args&&... args)
      : ScopedContextBase(std::forward<Args>(args)...) {}

  void synchronizeStreams(int dataDevice, cudaStream_t dataStream,
...
/**
 * The aim of this class is to do necessary per-event "initialization" in
 * ExternalWork acquire():
...
public:
  /// Constructor to create a new CUDA stream (no need for context beyond
  /// acquire())
  explicit ScopedContextAcquire(
      edm::StreamID streamID, edm::WaitingTaskWithArenaHolder waitingTaskHolder)
      : ScopedContextGetterBase(streamID),
        holderHelper_{std::move(waitingTaskHolder)} {}

  /// Constructor to create a new CUDA stream, and the context is needed after
  /// acquire()
  explicit ScopedContextAcquire(
      edm::StreamID streamID, edm::WaitingTaskWithArenaHolder waitingTaskHolder,
      ContextState& state)
  ...
  /// Constructor to (possibly) re-use a CUDA stream (no need for context beyond
  /// acquire())
  explicit ScopedContextAcquire(
  ...
  /// Constructor to (possibly) re-use a CUDA stream, and the context is needed
  /// after acquire()
  explicit ScopedContextAcquire(
      const ProductBase& data,
      edm::WaitingTaskWithArenaHolder waitingTaskHolder, ContextState& state)
      : ScopedContextGetterBase(data),
        holderHelper_{std::move(waitingTaskHolder)},
        contextState_{&state} {}
...
/**
 * The aim of this class is to do necessary per-event "initialization" in
 * ExternalWork produce() or normal produce():
 * - setting the current device
 * - synchronizing between CUDA streams if necessary
 * and enforce that those get done in a proper way in RAII fashion.
 */
...
public:
  ...
  /// Record the CUDA event, all asynchronous work must have been queued before
  /// the destructor
  ...
  std::unique_ptr<Product<T>> wrap(T data) {
    ...
    return std::unique_ptr<Product<T>>(
        new Product<T>(device(), streamPtr(), event_, std::move(data)));
  }

  template <typename T, typename... Args>
  auto emplace(edm::Event& iEvent, edm::EDPutTokenT<T> token, Args&&... args) {
    return iEvent.emplace(token, device(), streamPtr(), event_,
                          std::forward<Args>(args)...);
  }

private:
  friend class cuda::TestScopedContext;
...
/**
 * The aim of this class is to do necessary per-task "initialization" tasks
 * created in ExternalWork acquire():
 * - setting the current device
 * - calling edm::WaitingTaskWithArenaHolder::doneWaiting() when necessary
 * and enforce that those get done in a proper way in RAII fashion.
 */
...
public:
  /// Constructor to re-use the CUDA stream of acquire() (ExternalWork module)
  explicit ScopedContextTask(ContextState const* state,
                             edm::WaitingTaskWithArenaHolder waitingTaskHolder)
      : ScopedContextBase(
            state->device(),
            state->streamPtr()),  // don't move, state is re-used afterwards
        holderHelper_{std::move(waitingTaskHolder)},
        contextState_{state} {}
...
public:
  ...
  explicit ScopedContextAnalyze(const ProductBase& data)
      : ScopedContextGetterBase(data) {}
};
