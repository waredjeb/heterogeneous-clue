...
WaitingTaskWithArenaHolder::WaitingTaskWithArenaHolder(tbb::task_group& iGroup,
                                                       WaitingTask* iTask)
    : m_task(iTask),
      m_group(&iGroup),
  ...
  m_handle =
      std::make_shared<tbb::task_handle>(m_group->defer([task = m_task]() {
        TaskSentry s(task);
        task->execute();
      }));
}
...
WaitingTaskWithArenaHolder::WaitingTaskWithArenaHolder(
    WaitingTaskWithArenaHolder const& iHolder)
    : m_task(iHolder.m_task),
      m_group(iHolder.m_group),
      m_handle(iHolder.m_handle),
...
WaitingTaskWithArenaHolder::WaitingTaskWithArenaHolder(
    WaitingTaskWithArenaHolder&& iOther)
...
WaitingTaskWithArenaHolder& WaitingTaskWithArenaHolder::operator=(
    const WaitingTaskWithArenaHolder& iRHS) {
...
WaitingTaskWithArenaHolder& WaitingTaskWithArenaHolder::operator=(
    WaitingTaskWithArenaHolder&& iRHS) {
...
bool WaitingTaskWithArenaHolder::taskHasFailed() const noexcept {
  return m_task->exceptionPtr() != nullptr;
}

bool WaitingTaskWithArenaHolder::hasTask() const noexcept {
