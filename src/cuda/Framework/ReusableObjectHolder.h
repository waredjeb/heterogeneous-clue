...
//
/**\class edm::ReusableObjectHolder ReusableObjectHolder
 "ReusableObjectHolder.h"

 Description: Thread safe way to do create and reuse a group of the same object
 type.

 Usage:
 This class can be used to safely reuse a series of objects created on demand.
 The reuse
 of the objects is safe even across different threads since one can safely call
 all member
 functions of this class on the same instance of this class from multiple
 threads.

 This class manages the cache of reusable objects and therefore an instance of
 this
 ...
 NOTE: If you hold onto the std::shared_ptr<> until another call to the
 ReusableObjectHolder,
 make sure to release the shared_ptr before the call. That way the object you
 were just
 using can go back into the cache and be reused for the call you are going to
 make.
 An example
 \code
...
public:
  ...
  /// If there is an object already available, passes the object to iClearFunc
  /// and then
  /// returns the object.
  /// If there is not an object already available, creates a new one using
  /// iMakeFunc
...
private:
  std::unique_ptr<T> makeUnique(T* ptr) {
    static_assert(std::is_same_v<Deleter, std::default_delete<T>>,
                  "Generating functions returning raw pointers are supported "
                  "only with std::default_delete<T>");
