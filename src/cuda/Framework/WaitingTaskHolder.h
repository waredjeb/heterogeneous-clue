...
public:
  ...
  explicit WaitingTaskHolder(tbb::task_group& iGroup, edm::WaitingTask* iTask)
  ...
  WaitingTaskHolder(const WaitingTaskHolder& iHolder)
      : m_task(iHolder.m_task), m_group(iHolder.m_group) {
    m_task->increment_ref_count();
  }

  WaitingTaskHolder(WaitingTaskHolder&& iOther)
      : m_task(iOther.m_task), m_group(iOther.m_group) {
  ...
  /** since tbb::task_group is thread safe, we can return it non-const from here
     since
