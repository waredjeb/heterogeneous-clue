...
//
/**\class WaitingTaskList WaitingTaskList.h
 FWCore/Concurrency/interface/WaitingTaskList.h

 Description: Handles starting tasks once some resource becomes available.

 Usage:
    This class can be used to have tasks wait to be spawned until a resource is
 available.
 Tasks that want to use the resource are added to the list by calling
 add(tbb::task*).
 When the resource becomes available one calls doneWaiting() and then any
 waiting tasks will
 be spawned. If a call to add() is made after doneWaiting() the newly added task
 will
 immediately be spawned.
 The class can be reused by calling reset(). However, reset() is not thread-safe
 so one
 must be certain neither add(...) nor doneWaiting() is called while reset() is
 running.

 An example usage would be if you had a task doing a long calculation (the
 resource) and
 then several other tasks have been created in a different thread and before
 running those
 ...
 Finally in some unrelated part of the code we can create tasks that need the
 calculation
 \code
...
public:
  ...
  /**The WaitingTaskList is initial set to waiting.
     * \param[in] iInitialSize specifies the initial size of the cache used to
   * hold waiting tasks.
     * The value is only useful for optimization as the object can resize
   * itself.
     */
  explicit WaitingTaskList(unsigned int iInitialSize = 2);
  WaitingTaskList(const WaitingTaskList&) = delete;  // stop default
  const WaitingTaskList& operator=(const WaitingTaskList&) =
      delete;  // stop default
  ...
  /**If doneWaiting() has already been called then the added task will
   * immediately be spawned.
     * If that is not the case then the task will be held until doneWaiting() is
   * called and will
     * then be spawned.
  ...
  /**The owner of the resouce calls reset() to make tasks wait.
     * Calling reset() is NOT thread safe. The system must guarantee that no
   * tasks are
     * using the resource when reset() is called and neither add() nor
   * doneWaiting() can
