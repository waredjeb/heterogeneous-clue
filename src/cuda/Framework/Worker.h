...
public:
  ...
  // not thread safe
  void setItemsToGet(std::vector<Worker*> workers) {
    itemsToGet_ = std::move(workers);
  }

  // thread safe
  void prefetchAsync(Event& event, EventSetup const& eventSetup,
                     WaitingTaskHolder iTask);

  // not thread safe
  virtual void doWorkAsync(Event& event, EventSetup const& eventSetup,
                           WaitingTaskHolder iTask) = 0;
...
public:
  ...
  void doWorkAsync(Event& event, EventSetup const& eventSetup,
                   WaitingTaskHolder task) override {
    waitingTasksWork_.add(task);
    // std::cout << "doWorkAsync for " << this << " with iTask " << iTask <<
    // std::endl;
    bool expected = false;
    if (workStarted_.compare_exchange_strong(expected, true)) {
      ...
      WaitingTask* moduleTask = make_waiting_task(
          [this, &event, &eventSetup](std::exception_ptr const* iPtr) mutable {
            if (iPtr) {
              ...
              // std::cout << "waitingTasksWork_.doneWaiting " << this <<
              // std::endl;
      ...
      // std::cout << "calling prefetchAsync " << this << " with moduleTask " <<
      // moduleTask << std::endl;
