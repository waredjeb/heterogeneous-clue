...
public:
  ...
  virtual std::unique_ptr<ESProducer> create(
      std::filesystem::path const& datadir) const = 0;
};
...
public:
  virtual std::unique_ptr<ESProducer> create(
      std::filesystem::path const& datadir) const override {
...
public:
  Registrar(std::string const& name) {
    getGlobalRegistry().add(name, std::make_unique<Maker<T>>());
  }
};
}  // namespace impl

std::unique_ptr<ESProducer> create(std::string const& name,
...
#define DEFINE_FWK_EVENTSETUP_MODULE(type)                              \
  static edm::ESPluginFactory::impl::Registrar<type> EDM_ES_PLUGIN_SYM( \
      maker, __LINE__)(#type);
