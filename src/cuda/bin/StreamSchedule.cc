...
StreamSchedule::StreamSchedule(ProductRegistry reg,
                               ...
                               Source* source, EventSetup const* eventSetup,
                               int streamId,
                               std::vector<std::string> const& path)
    : registry_(std::move(reg)),
      source_(source),
      eventSetup_(eventSetup),
      streamId_(streamId) {
...
void StreamSchedule::runToCompletionAsync(WaitingTaskHolder h) {
  auto task = make_functor_task([this, h]() mutable {
...
void StreamSchedule::processOneEventAsync(WaitingTaskHolder h) {
  ...
  if (event) {
    ...
    for (auto iWorker = path_.rbegin(); iWorker != path_.rend(); ++iWorker) {
      // std::cout << "calling doWorkAsync for " << iWorker->get() << " with
      // nextEventTask " << nextEventTask << std::endl;
