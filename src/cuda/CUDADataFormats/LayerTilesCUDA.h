...
public:
  ...
  __device__ void fill(const std::vector<float>& x,
  ...
  __host__ __device__ int getXBin(float x) const {
    ...
    xBin = (xBin < LayerTilesConstants::nColumns
                ? xBin
                : LayerTilesConstants::nColumns - 1);
    xBin = (xBin > 0 ? xBin : 0);
    return xBin;
  }
  ...
  __host__ __device__ int getYBin(float y) const {
    ...
    yBin = (yBin < LayerTilesConstants::nRows ? yBin
                                              : LayerTilesConstants::nRows - 1);
    yBin = (yBin > 0 ? yBin : 0);
    ;
    return yBin;
  }
  ...
  __host__ __device__ int4
  searchBox(float xMin, float xMax, float yMin, float yMax) {
  ...
  __host__ __device__ CUDAVect& operator[](int globalBinId) {
    return layerTiles_[globalBinId];
  }

private:
  cms::cuda::VecArray<CUDAVect, LayerTilesConstants::nColumns*
                                    LayerTilesConstants::nRows> layerTiles_;
};
// }  // namespace ALPAKA_ACCELERATOR_NAMESPACE
