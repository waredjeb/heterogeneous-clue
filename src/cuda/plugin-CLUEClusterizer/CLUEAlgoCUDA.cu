...
void CLUEAlgoCUDA::setup(PointsCloud const& host_pc) {
  ...
  cudaCheck(cudaMemcpyAsync(d_points.x.get(), host_pc.x.data(), sizeof(float) * host_pc.n,
             cudaMemcpyHostToDevice , stream_));
  cudaCheck(cudaMemcpyAsync(d_points.y.get(), host_pc.y.data(), sizeof(float) * host_pc.n,
             cudaMemcpyHostToDevice, stream_ ));
  cudaCheck(cudaMemcpyAsync(d_points.layer.get(), host_pc.layer.data(),
             sizeof(int) * host_pc.n, cudaMemcpyHostToDevice, stream_ ));
  cudaCheck(cudaMemcpyAsync(d_points.weight.get(), host_pc.weight.data(),
             sizeof(float) * host_pc.n, cudaMemcpyHostToDevice, stream_ ) );
  ...
  cudaCheck(cudaMemsetAsync(d_followers.get(), 0x00,
